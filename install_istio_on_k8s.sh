...
1. 需要给端口正确命名：服务端口必须进行命名。端口名称只允许是<协议>[-<后缀>-]模式，其中<协议>部分可选择范围包括 http、http2、grpc、mongo 以及 redis，Istio 可以通过对这>些协议的支持来提供路由能力。例如 name: http2-foo 和 name: http 都是有效的端口名，但 name: http2foo 就是无效的。如果没有给端口进行命名，或者命名没有使用指定前缀，那么这>一端口的流量就会被视为普通 TCP 流量（除非显式的用 Protocol: UDP 声明该端口是 UDP 端口）。

2. 关联服务：Pod 必须关联到 Kubernetes 服务，如果一个 Pod 属于多个服务，这些服务不能再同一端口上使用不同协议，例如 HTTP 和 TCP。

3. Deployment 应带有 app 以及 version 标签：在使用 Kubernetes Deployment 进行 Pod 部署的时候，建议显式的为 Deployment 加上 app 以及 version 标签。每个 Deployment 都应该
有一个有意义的 app 标签和一个用于标识 Deployment 版本的 version 标签。app 标签在分布式跟踪的过程中会被用来加入上下文信息。Istio 还会用 app 和 version 标签来给遥测指标数
据加入上下文信息。
"
...
EOF
    ...
    refresh_env=`source $HOME/.bashrc`
    ...
    echo "ISTIO INSTALL PATH: "$ISTIO_PATH
    istioctl version
    echo "*********************************************************"
    sleep 1
fi

ISTIO_HOME="$ISTIO_PATH"
# download helm
...
EOF
    ...
    echo "HELM INSTALL PATH: /usr/local/helm"
    helm version
    echo -e "\n***************************************************************************"
    sleep 1
else
...
helm version > /dev/null 2>&1
if [ $? -eq 127 ];then 
    echo '环境变量刷新失败,请执行"source $HOME/.bashrc"命令来刷新环境变量,然后重新执行该脚本即可.'
fi
...
# 默认情况下，Istio 使用 负载均衡器 服务对象类型。有些平台不支持 负载均衡器 服务对象类型。对于缺少 负载均衡器 支持的平台，安装需要带有 “NodePort” 支持的 Istio
helm install $ISTIO_HOME/install/kubernetes/helm/istio --name istio --namespace istio-system --set gateways.istio-ingressgateway.type=NodePort --set gateways.istio-egressgateway.type=NodePort
